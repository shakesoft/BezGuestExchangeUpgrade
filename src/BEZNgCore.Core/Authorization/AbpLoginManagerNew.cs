...
namespace BEZNgCore.Authorization
{
      ...
      where TUser : AbpUser<TUser>
    {
        ...
        [UnitOfWork]
        public virtual async Task<AbpLoginResult<TTenant, TUser>> LoginAsyncNew(string userNameOrEmailAddress, string plainPassword, string tenancyName = null, bool shouldLockout = true)
        {
            var result = await LoginAsyncInternalNew(userNameOrEmailAddress, plainPassword, tenancyName, shouldLockout);
            await SaveLoginAttemptAsync(result, tenancyName, userNameOrEmailAddress);
            return result;
        }
        protected virtual async Task<AbpLoginResult<TTenant, TUser>> LoginAsyncInternalNew(string userNameOrEmailAddress, string plainPassword, string tenancyName, bool shouldLockout)
        {
                ...
                if (!userNameOrEmailAddress.IsNullOrEmpty())
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // return new AbpLoginResult<TTenant, TUser>(AbpLoginResultType.InvalidUserNameOrEmailAddress, tenant);
                                throw new UserFriendlyException("Incorrect UID or Password.");
        ...
        [UnitOfWork]
        public virtual async Task<AbpLoginResult<TTenant, TUser>> LoginAsyncGuestExchange(string userNameOrEmailAddress, string plainPassword, string tenancyName = null, bool shouldLockout = true)
        {
            var result = await LoginAsyncInternalGuestExchange(userNameOrEmailAddress, plainPassword, tenancyName, shouldLockout);
            await SaveLoginAttemptAsync(result, tenancyName, userNameOrEmailAddress);
            return result;
        }
        protected virtual async Task<AbpLoginResult<TTenant, TUser>> LoginAsyncInternalGuestExchange(string userNameOrEmailAddress, string plainPassword, string tenancyName, bool shouldLockout)
        {
            //Get and check tenant
            TTenant tenant = null;
            using (UnitOfWorkManager.Current.SetTenantId(null))
            ...
            using (UnitOfWorkManager.Current.SetTenantId(tenantId))
            {
                ...
                if (!plainPassword.IsNullOrEmpty() && !userNameOrEmailAddress.IsNullOrEmpty())
                {
                    var psw = Base64Encryption(plainPassword);
                    var user = (from u in _userRepository.GetAll()
                                where (u.EmailAddress == userNameOrEmailAddress || u.UserName == userNameOrEmailAddress || u.PhoneNumber == userNameOrEmailAddress)                                 
                                && u.Password==psw
                                select u).SingleOrDefault();
        ...
        {
            const string DefaultTenantName = "TEST";
